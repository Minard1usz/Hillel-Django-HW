...
class CartViewSet(viewsets.ViewSet):
    ...
    def add_item(self, request):
        ...
        if serializer.is_valid():
            ...
            return Response({'status': 'Кошик оновлено успішно'}, status=status.HTTP_200_OK)
    ...
    def remove_item(self, request):
        ...
        if not book_id:
            return Response({'error': 'Параметр book_id обов\'язковий'}, status=status.HTTP_400_BAD_REQUEST)

        try:
            book = Book.objects.get(pk=book_id)
            cart = Cart(request)
            cart.remove(book)
            return Response({'status': 'Товар видалено з кошика'}, status=status.HTTP_200_OK)
            ...
            with transaction.atomic():
                ...
                for item in cart:
                    ...
                    if book.stock < item['quantity']:
                        raise ValueError(f"Недостатньо товару '{book.title}' на складі. Доступно: {book.stock}")
